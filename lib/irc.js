 ...
 *          irc.msg('Hi chat!');
 *          irc.msg('Hi chat!', {caller: 'aatxxe', auth: 1});
 *
 *      get user name:
 *          irc.caller(data[0]);
 */
...
function IRC(options) {
  ...
  __self.config = {
    ...
    nick: 'irc_bot',
    broadcaster:
      __self.options.channel.slice(1).charAt(0).toUpperCase() + __self.options.channel.slice(2).toLowerCase(),
    // twitch server
...
IRC.prototype.connect = function () {
  ...
  __self.socket.on('connect', function () {
    ...
    __self.raw(
      'USER ' + __self.config.nick + ' ' + __self.config.nick + '.com ' + __self.config.nick + ' :' + __self.config.name
    )
  })
...
IRC.prototype.msg = function (msg, options) {
  var __self = this,
    opts = options || { caller: null, auth: 0 }
